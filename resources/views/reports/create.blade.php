...
@section('description', 'Add a new cable disruption incident report')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h3 class="text-lg font-medium text-gray-900">Repair Report Details</h3>
                <p class="text-sm text-gray-600">Fill in the information about the site and cable repair report</p>
            </div>

            <form action="{{ route('reports.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6" id="reportForm">
                @csrf
                ...
                <div>
                    <label for="work_details" class="block text-sm font-medium text-gray-700 mb-2">
                        Work Details <span class="text-red-500">*</span>
                    </label>
                    <textarea name="work_details" id="work_details" rows="4"
                        ...
                        placeholder="Describe the work details and incident information">{{ old('work_details') }}</textarea>
                    @error('work_details')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="longitude" id="longitude" step="any" value="{{ old('longitude') }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors duration-200 @error('longitude') border-red-500 @enderror"
                ...
                <!-- Location Status -->
                <p id="location-status" class="text-center text-sm text-gray-500 mt-2"></p>

                <!-- Solution Type with custom dropdown -->
                <div>
                    <label for="repair_type" class="block text-sm font-medium text-gray-700 mb-2">
                        Repair Type <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <input type="hidden" name="repair_type" id="repair_type" value="{{ old('repair_type') }}">
                        <button type="button" 
                                id="solutionDropdownButton"
                                class="inline-flex items-center justify-between w-full px-4 py-3 border border-gray-300 rounded-xl bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors duration-200 @error('repair_type') border-red-600 ring-2 ring-red-500 @enderror">
                            <span id="solutionSelectedText">Select repair type</span>
                            <i class="fas fa-chevron-down ml-2 text-gray-400 transition-transform duration-200" id="solutionChevron"></i>
                        </button>
                ...
                <!-- Cable Type with custom dropdown -->
                <div>
                    <label for="cable_type" class="block text-sm font-medium text-gray-700 mb-2">
                        Cable Type <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <input type="hidden" name="cable_type" id="cable_type" value="{{ old('cable_type') }}">
                        <button type="button" 
                                id="cableDropdownButton"
                                class="inline-flex items-center justify-between w-full px-4 py-3 border border-gray-300 rounded-xl bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors duration-200 @error('cable_type') border-red-600 ring-2 ring-red-500 @enderror">
                            <span id="cableSelectedText">Select cable type</span>
                            <i class="fas fa-chevron-down ml-2 text-gray-400 transition-transform duration-200" id="cableChevron"></i>
                        </button>
                ...
                <!-- Cause of Disruption with custom dropdown -->
                <div>
                    ...
                    <div class="relative">
                        <input type="hidden" name="disruption_cause" id="disruption_cause" value="{{ old('disruption_cause') }}">
                        <button type="button" 
                                id="causeDropdownButton"
                                class="inline-flex items-center justify-between w-full px-4 py-3 border border-gray-300 rounded-xl bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors duration-200 @error('disruption_cause') border-red-600 ring-2 ring-red-500 @enderror">
                            <span id="causeSelectedText">Select cause of disruption</span>
                            <i class="fas fa-chevron-down ml-2 text-gray-400 transition-transform duration-200" id="causeChevron"></i>
                        </button>
                        ...
                        <div id="causeDropdownMenu" 
                            ...
                            <div class="py-1">
                                <button type="button" 
                                    ...
                                    <span class="flex items-center">
                                        <i class="fas fa-hammer text-red-600 mr-2"></i>
                                        Vandalism
                                    </span>
                                ...
                                <button type="button" 
                                    ...
                                    <i class="fas fa-check text-red-500 opacity-0" id="check-cause-thirdparty"></i>
                                </button>
                                <button type="button" 
                                        class="cause-option w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors duration-150 flex items-center justify-between"
                ...
                <!-- Related Reports with improved styling -->
                <div>
                    <label for="related_reports" class="block text-sm font-medium text-gray-700 mb-2">
                        Related Reports
                    </label>
                    <div class="relative">
                        <select name="related_reports[]" id="related_reports" multiple
                            ...
                            @foreach($existingReports as $existingReport)
                                <option value="{{ $existingReport->id_repair_reports }}" 
                                    {{ in_array($existingReport->id_repair_reports, old('related_reports', [])) ? 'selected' : '' }}
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-red-400 transition-colors">
                        <div class="space-y-1 text-center">
                            <div id="images-preview" class="hidden grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                            </div>
                            <div id="upload-placeholder" class="flex flex-col items-center justify-center rounded-lg p-6 text-center">
                                ...
                                <div class="flex text-sm text-gray-600 items-center justify-center">
                                    <label for="documentation"
                                        ...
                                        <input id="documentation" name="documentation[]" type="file" class="sr-only"
                                            accept="image/*" multiple>
                                    </label>
                                    <p class="pl-1">or drag and drop</p>
                                </div>
                                <p class="text-xs text-gray-500 mt-2">
                                    PNG, JPG, GIF up to 2MB each (multiple files allowed)
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const oldSolution = "{{ old('repair_type') }}";
                const oldCable = "{{ old('cable_type') }}";
                const oldCause = "{{ old('disruption_cause') }}";
                
                if (oldSolution) updateSolutionSelection(oldSolution);
                if (oldCable) updateCableSelection(oldCable);
                if (oldCause) updateCauseSelection(oldCause);
                ...
                function updateCauseSelection(value, text = null) {
                    ...
                    const causeMap = {
                        'Vandalism': 'check-cause-vandalism',
                        'Animal Disturbance': 'check-cause-animal',
                        'Third Party Activity': 'check-cause-thirdparty',
                        'Natural Disturbance': 'check-cause-natural',
                        'Electrical Issue': 'check-cause-electrical',
                        'Traffic Accident': 'check-cause-traffic'
                    };
                    
                    if (causeMap[value]) {
                        causeSelectedText.textContent = text || value;
                        const checkElement = document.getElementById(causeMap[value]);
                        if (checkElement) {
